# MXDL-1-05] 5.linear_activation.py
#
...
#
# Even with a multi-layer perceptron, nonlinear problems 
# cannot be solved if a linear activation function is used 
# in the hidden layer.
...
plt.scatter(x[:, 0], x[:, 1], s=80, c=color, alpha=0.3)
plt.show()
...
def predict(x, proba=True):
    p = parameters
    h_out = np.dot(x, p[0]) + p[1]     # linear output from hidden layer
